...
export const getAnalyticsData = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // 3. Parallel fetching of org-scoped data — date filters applied at DB level
    // to avoid transferring all rows for large orgs. "all" preset fetches without
    // date bounds for full aggregate accuracy.
    const prevStartIso = previousStart.toISOString();
    const currentEndIso = currentEnd.toISOString();
    const isAllPreset = data.dateRange === "all";
    ...
    const [
      ...
      // Invoices: fetch from previousStart to now (covers both comparison windows)
      (() => {
        let q = supabase
          .from("invoices")
          .select("id, number, amount, status, customer_id, issue_date, due_date, paid_at, created_at")
          .eq("org_id", data.orgId);
        if (!isAllPreset) q = q.gte("issue_date", prevStartIso.split("T")[0]);
        return q;
      })(),
      // Deals: fetch from previousStart to currentEnd
      (() => {
        let q = supabase
          .from("deals")
          .select("id, title, value, stage, expected_close, customer_id, created_at")
          .eq("org_id", data.orgId);
        if (!isAllPreset) q = q.gte("created_at", prevStartIso);
        return q;
      })(),
      // Customers: all-time counts are needed for customer analytics — no date filter
      supabase
        .from("customers")
        .select("id, name, company, email, status, created_at")
        .eq("org_id", data.orgId),
      // Tasks: fetch from previousStart
      (() => {
        let q = supabase
          .from("tasks")
      ...
      // Leads: fetch from previousStart
      (() => {
        let q = supabase
          .from("leads")
    ...
    if (tasksRes.error && !tasksRes.error.message.includes("schema cache") && !tasksRes.error.message.includes("does not exist")) {
      throw new Error(`Tasks error: ${tasksRes.error.message}`);
    }
    if (leadsRes.error && !leadsRes.error.message.includes("schema cache") && !leadsRes.error.message.includes("does not exist")) {
    ...
    if (overdueRevenue > 0) {
      ...
      insights.push({
        ...
        actionRecommended: "Send automated invoice payment reminders via WhatsApp or email.",
        generatedBy: "ai-grounded-engine",
      });
    }
