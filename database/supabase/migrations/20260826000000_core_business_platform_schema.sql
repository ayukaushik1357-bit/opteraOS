...
-- Note: departments table was created by migration 20260815100000 with column
-- 'org_id'. We rename it to 'organization_id' for consistency across the platform.
DO $$
BEGIN
  -- Rename org_id → organization_id if the old name still exists
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'departments' AND column_name = 'org_id'
  ) AND NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'departments' AND column_name = 'organization_id'
  ) THEN
    ALTER TABLE public.departments RENAME COLUMN org_id TO organization_id;
  END IF;
END;
$$;
...
-- Note: teams table was also created with 'org_id'; rename for consistency.
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    ...
    ALTER TABLE public.teams RENAME COLUMN org_id TO organization_id;
  END IF;
END;
$$;
...
-- notifications.read is the actual boolean column (created by 20260814100000)
CREATE INDEX IF NOT EXISTS idx_notifications_unread ON public.notifications(user_id, read);

-- audit_logs: create if not exists, then augment
