...
export const createOrganization = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((data) =>
    z.object({ name: z.string().trim().min(2).max(60), currency: z.string().trim().min(3).max(3) }).parse(data),
  )
...
export const acceptInvite = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await context.supabase.from("organization_invites").update({ status: "accepted" }).eq("id", invite.id);
...
export const getDashboard = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    const [customers, deals, invoices] = await Promise.all([
      context.supabase.from("customers").select("id, name, company, status, created_at").eq("org_id", data.orgId),
      context.supabase.from("deals").select("id, title, value, stage, expected_close, created_at").eq("org_id", data.orgId),
      context.supabase
        .from("invoices")
    ...
    return {
      ...
      revenueSeries: months.map((m) => ({ month: m.label, revenue: revenueByMonth.get(m.key) ?? 0 })),
      pipelineByStage: ["lead", "qualified", "proposal", "negotiation"].map((stage) => ({
        stage,
        value: dealRows.filter((d) => d.stage === stage).reduce((s, d) => s + Number(d.value ?? 0), 0),
...
export const revokeInvite = createServerFn({ method: "POST" })
  ...
  .inputValidator((data) => z.object({ inviteId: z.string().uuid() }).parse(data))
  .handler(async ({ data, context }) => {
    const { error } = await context.supabase.from("organization_invites").delete().eq("id", data.inviteId);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
