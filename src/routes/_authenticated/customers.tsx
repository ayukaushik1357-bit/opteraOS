...
const description = "Manage every customer relationship, contact detail and account status inside your opteraOS CRM.";
...
function CustomersPage() {
  ...
  const [editing, setEditing] = useState<{ id?: string; name: string; company: string; email: string; phone: string; status: Status }>(
    ...
    <div className="grid gap-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Your CRM records, shared across the workspace.</p>
        </div>
          ...
          <DialogContent>
            ...
            <div className="grid gap-4">
              <div className="grid gap-2">
                ...
                <Input id="c-name" value={editing.name} onChange={(e) => setEditing({ ...editing, name: e.target.value })} />
              </div>
              <div className="grid gap-2">
                <Label htmlFor="c-company">Company</Label>
                <Input id="c-company" value={editing.company} onChange={(e) => setEditing({ ...editing, company: e.target.value })} />
              </div>
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="grid gap-2">
                  <Label htmlFor="c-email">Email</Label>
                  <Input id="c-email" type="email" value={editing.email} onChange={(e) => setEditing({ ...editing, email: e.target.value })} />
                </div>
                <div className="grid gap-2">
                  <Label htmlFor="c-phone">Phone</Label>
                  <Input id="c-phone" value={editing.phone} onChange={(e) => setEditing({ ...editing, phone: e.target.value })} />
                </div>
              </div>
              <div className="grid gap-2">
                <Label>Status</Label>
      ...
      <div className="glass overflow-hidden rounded-2xl">
        {isLoading ? (
          ...
          <p className="p-8 text-center text-sm text-muted-foreground">No customers yet — add your first one.</p>
        ) : (
          <Table>
            <TableHeader>
              <TableRow>
              ...
              {(data ?? []).map((c) => (
                <TableRow key={c.id}>
                  ...
                  <TableCell className="text-right">
                    ...
                    <Button variant="ghost" size="sm" onClick={() => deleteMutation.mutate(c.id)} aria-label={`Delete ${c.name}`}>
