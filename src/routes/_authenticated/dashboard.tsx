...
import { AlertTriangle, IndianRupee, TrendingUp, Users, Wallet } from "lucide-react";
import { Skeleton } from "@/components/ui/skeleton";
import { useWorkspace } from "@/components/app/AppShell";
import { getDashboard } from "@/lib/workspace.functions";

const title = "Overview — opteraOS";
const description = "Live revenue, pipeline, invoice and customer KPIs for your business, unified in the opteraOS overview.";
...
function Dashboard() {
  ...
  const cards = [
    { label: "Revenue collected", value: money(k.revenue, currency), sub: `${k.overdueInvoices} overdue invoices`, icon: IndianRupee },
    { label: "Outstanding", value: money(k.outstanding, currency), sub: "Sent + overdue invoices", icon: Wallet },
    { label: "Open pipeline", value: money(k.pipeline, currency), sub: `${k.openDeals} open deals · ${k.winRate}% win rate`, icon: TrendingUp },
    { label: "Customers", value: String(k.customers), sub: `${k.activeCustomers} active`, icon: Users },
  ];
    ...
    <div className="grid gap-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Live overview across sales, invoices and customers.
        </p>
      </div>
      ...
      {empty && (
        <div className="flex items-start gap-3 rounded-2xl border border-border bg-secondary/30 p-4 text-sm text-muted-foreground">
          ...
          <p>
            Your workspace is empty, so every KPI reads zero. As customers, deals and invoices are added, this
          ...
          <div className="mt-4 h-64">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={data.revenueSeries}>
                ...
                />
                <Area type="monotone" dataKey="revenue" stroke="var(--brand-blue)" strokeWidth={2} fill="url(#revFill)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
          ...
          {data.recentInvoices.length === 0 ? (
            ...
            <ul className="mt-4 grid gap-2">
              {data.recentInvoices.map((inv) => (
                <li key={inv.id} className="flex items-center justify-between rounded-xl bg-secondary/40 px-3 py-2 text-sm">
          ...
          {data.recentDeals.length === 0 ? (
            ...
            <ul className="mt-4 grid gap-2">
              {data.recentDeals.map((deal) => (
                <li key={deal.id} className="flex items-center justify-between rounded-xl bg-secondary/40 px-3 py-2 text-sm">
